...
import { loadProgress, recordAnswer, saveProgress } from "@/entities/progress";
import { buildDeck } from "../model/build-deck";
import { xpFor, streakBonus } from "../model/score";

const SESSION_SIZE = 5;

export function SessionScreen() {
  // 덱은 마운트 시점의 진도로 한 번만 만든다.
  // 세션 도중 진도가 바뀌어도 출제 순서가 흔들리지 않게.
  const [deck] = useState(() =>
    buildDeck(questions, concepts, loadProgress(), SESSION_SIZE),
  );
  ...
  function pick(choiceId: string) {
    ...
    const gained = correct ? xpFor(question) + streakBonus(nextStreak) : 0;

    setStreak(nextStreak);
    setSessionXp(sessionXp + gained);
    if (!correct) setWrongConceptIds([...wrongConceptIds, question.conceptId]);

    const updated = recordAnswer(progress, {
  ...
  if (done) {
    ...
    const wrongNames = [...new Set(wrongConceptIds)].map(
      (id) => concepts.find((concept) => concept.id === id)?.name ?? id,
    );
    ...
    return (
      <main className="mx-auto flex w-full max-w-xl flex-1 flex-col justify-center gap-8 px-4 py-10">
        <div className="text-center">
          ...
          <p className="mt-3 text-lg font-medium text-emerald-600">
            +{sessionXp} XP
          </p>
        </div>
        ...
        {wrongNames.length > 0 && (
          <div className="rounded-xl border border-neutral-200 p-4 dark:border-neutral-800">
            <p className="text-sm font-medium text-neutral-500">다시 볼 개념</p>
  ...
  if (!question) {
    return (
      <main className="mx-auto flex w-full max-w-xl flex-1 flex-col justify-center gap-6 px-4 py-10 text-center">
        <p className="text-neutral-500">출제할 문항이 없습니다.</p>
      ...
      <div className="flex flex-1 flex-col gap-5">
        ...
        <div className="flex flex-col gap-3">
          {question.choices.map((choice) => {
            ...
            if (picked) {
              if (isAnswer)
                tone = "border-emerald-500 bg-emerald-50 dark:bg-emerald-950";
              else if (selected)
                tone = "border-red-500 bg-red-50 dark:bg-red-950";
